...
- [ ] ⚠️ Renewing/deleting IPv6 port maps does not work; adding and expiration works  
  Resulting in IPv6 port maps expiring regularly. `ioctl(dev, DIOCCHANGERULE, ...) PF_CHANGE_GET_TICKET: Invalid argument` gets logged. pf renewing not yet implemented, deleting not working: https://github.com/miniupnp/miniupnp/issues/747
...
- [ ] Logging issues and recommendations
  ...
  - Daemon does not log IPv4/IPv6 mapping requests clearly  
    ...
    ```
    Add IPv4 port map 10000:192.168.1.100:10000/TCP remote_ip=2.1.2.3 lifetime=3600 via=UPnP IGD
    Renew IPv6 port map [2001:db8::100]:10000/TCP remote_ip=2123::/56 remote_port=1234 lifetime=3600 via=UPnP-IGDv2-IPv6[-Update] uid=…
    Reject IPv6 port map [2001:db8::100]:10001/TCP lifetime=3600 via=PCP nonce=… reason=ACL/port-in-use/for-third-party…
    Delete/Expire (similar to add)
    ```
...
- [ ] Renewing an IPv4 port map with a remote IP no longer works, error 718 ConflictInMappingEntry is returned, currently in Git head
- [ ] Remote IP filter for IPv4 port maps are lost on lease file reloads, in Git head not backwards compatible fixed  
  - Current lease file functionality had/has many issues and should possibly be replaced at the same time by a new implementation that follows a standard of a unified IPv4/IPv6 port mapping lease file format specifying more fields, e.g. a random mapping ID for easy/reliable deletion in the first field, IP ver., more conveniently ordered fields and better separator, also more directly usable in UIs
  - For compatibility, it should be built-in if no (old) lease file support has been enabled (via `#ifndef ENABLE_LEASEFILE`). Using new format then ensures backwards compatibility, so that the switch to new unified IPv4/IPv6 format can be made downstream at any time, also as the IPv4 lease file format has not changed since introduction. Support for old format can therefore also be easily removed from daemon later.
  - Unified IPv4/IPv6 lease file format example, fields currently unavailable are added (reserved) for forward-compatibility:
     ```
     UUID (for unique deletion of entries) | Res. | IP ver | IP address    | Port  | Protocol | Res. | Res. | External port | Res. | Remote IP address | Remote port | Expires    | Added via       | IDs (UID/Nonce)          | Res. | Map description
     d4bd9441-122f-4837-aae1-f91fef5ac3aa  |      | IPv4   | 192.168.1.100 | 12345 | TCP      |      |      | 12345         |      | 198.51.100.1/24   | 10000       | 1704067200 | PCP             | f9b9e63857bff84fe9e857dd |      |
     ```
