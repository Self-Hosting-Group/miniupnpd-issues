...
- [ ] ❗ All IPv4 port maps expire (get deleted) on `reload_from_lease_file` since https://github.com/miniupnp/miniupnp/commit/671e5c0
- [X] ❗ Build failure with `make` (using musl) fails with `testupnphttp.c:97-98:9: error: unknown type name 'fd_set'` (in head)
...
- [ ] IPv4 port maps with remote IP filters set are lost on lease file reloads, in head not backwards compatible fixed  
  - Current lease file functionality had/has many issues and should possibly be replaced at the same time by a new implementation that follows a standard of a unified IPv4/IPv6 port mapping lease file format specifying more fields, e.g. a random mapping ID for easy/reliable deletion in the first field, IPv4/IPv6 family, and more conveniently ordered fields and more compatible `|` separator, directly usable in UIs/Markdown
  - For compatibility, it should be built-in if no (old) lease file support has been enabled. Using the new format only via `#ifndef ENABLE_LEASEFILE` ensures backwards compatibility, and that the switch to new unified IPv4/IPv6 lease file can be made downstream at a desired time. The IPv4 lease file format has not changed since introduction, and this makes it easy to remove the old support at a later time
  - At the moment, to avoid unexpected breaks soon (downstream devs would possibly be happy), a quick fix would be to replace two `#ifdef SUPPORT_REMOTEHOST` with `#if 0 // Disabled until 2.4.0 #ifdef SUPPORT_REMOTEHOST` in the code until 2.4.0, to not break in the next patch release, as many router UIs (OpenWrt/X-Wrt/ASUS) parse the file
...
Updated: 2026-09-07, tested with the latest commit `63ac33e` of the [daemon](https://github.com/miniupnp/miniupnp/commits/master/miniupnpd) from 2026-09-06
